...
describe('Derivation', () => {
  ...
  it('should derive consistent keypair from mnemonic', () => {
    const kp1 = Derivation.fromMnemonic(TEST_MNEMONIC);
    const kp2 = Derivation.fromMnemonic(TEST_MNEMONIC);
  ...
  it('should produce deterministic address from mnemonic', () => {
    const addr1 = Derivation.mnemonicToAddress(TEST_MNEMONIC);
    const addr2 = Derivation.mnemonicToAddress(TEST_MNEMONIC);

    expect(addr1).toBe(addr2);
  });

  it('should have 32-byte public key', () => {
    const kp = Derivation.fromMnemonic(TEST_MNEMONIC);
    expect(kp.publicKey).toHaveLength(32);
  });

  it('should have 64-byte secret key', () => {
    const kp = Derivation.fromMnemonic(TEST_MNEMONIC);
    expect(kp.secretKey).toHaveLength(64);
  });

  it('should reject invalid mnemonic', () => {
    expect(() => Derivation.fromMnemonic('invalid mnemonic')).toThrow();
  });
});
